...
bool IsInitialBlockDownload()
{
    ...
    bool state = (chainActive.Height() < pindexBestHeader->nHeight - 24 * 6 ||
            chainActive.Tip()->GetBlockTime() < GetTime() - chainParams.MaxTipAge()); // ~144 blocks behind -> 2 x fork detection time
    if (!state)
        lockIBDState = true;
    return state;
}
...
#endif
		if (pindexFrom->nHeight < pindex->nHeight - nDuration)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, CBlockIndex * const pindexPrev)
{
    ...
    if (chainActive.Height() - nHeight >= nMaxReorgDepth)
        return state.DoS(50, error("%s: forked chain older than max reorganization depth (height %d)", __func__, nHeight));
...
static bool AddGenesisBlock(const CBlock& block, CValidationState& state)
{
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        try {
            ...
            if (Params().NetworkID() == CBaseChainParams::DEVNET) {
                if (!AddGenesisBlock(Params().DevNetGenesisBlock(), state))
                    return false;
            }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (Params().NetworkID() == CBaseChainParams::DEVNET) {
            if (pfrom->strSubVer.find(strprintf("devnet=%s", GetDevNetName())) == std::string::npos) {
                LOCK(cs_main);
                LogPrintf("connected to wrong devnet. Reported version is %s, expected devnet name is %s\n", pfrom->strSubVer, GetDevNetName());
                if (!pfrom->fInbound)
...
{
    {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted < 2 && fFetch) || chainActive.Tip()->GetBlockTime() > GetAdjustedTime() - 6 * 60 * 60) {
