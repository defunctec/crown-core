...
public:
    explicit CRegTestParams(const RegTestOptions& opts)
    {
        ...
        // Intentionally reuse regtest genesis parameters during Phase 1 so the
        // experimental Crown chain keeps Bitcoin's existing PoW validation flow.
...
class CCrownParams : public CChainParams
{
public:
    CCrownParams()
    {
        m_chain_type = ChainType::CROWN;
        consensus.signet_blocks = false;
        consensus.signet_challenge.clear();
        consensus.nSubsidyHalvingInterval = 150;
        consensus.BIP34Height = 1;
...
std::unique_ptr<const CChainParams> CChainParams::Crown()
{
    return std::make_unique<const CCrownParams>();
}
...
std::optional<ChainType> GetNetworkForMagic(const MessageStartChars& message)
{
    ...
    const auto crown_msg = CChainParams::Crown()->MessageStart();
    ...
    } else if (std::ranges::equal(message, crown_msg)) {
        return ChainType::CROWN;
    }
    return std::nullopt;
}
