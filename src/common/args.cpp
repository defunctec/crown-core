...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ChainTypeToString(ChainType::CROWN),
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    const bool fCrown = get_net("-crown");
    const auto chain_arg = GetArg("-chain");

    if ((int)chain_arg.has_value() + (int)fRegTest + (int)fSigNet + (int)fTestNet + (int)fTestNet4 + (int)fCrown > 1) {
        throw std::runtime_error("Invalid combination of -regtest, -signet, -testnet, -testnet4, -crown and -chain. Can use at most one.");
    }
    if (chain_arg) {
    ...
    if (fCrown) return ChainType::CROWN;
