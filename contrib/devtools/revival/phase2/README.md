...
- `phase2-controlled-sync-check.sh`
  - Starts `crownd` with testnet/regtest explicitly disabled (`-testnet=0 -regtest=0`), IPv4-only loopback RPC binding, and inbound-disabled peer settings (`listen=0`, `upnp=0`), with outbound peer discovery explicitly enabled (`discover=1`, `dnsseed=1`, `dns=1`).
