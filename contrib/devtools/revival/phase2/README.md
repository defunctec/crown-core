...
- `phase2-controlled-sync-check.sh`
  - Starts `crownd` with testnet/regtest explicitly disabled (`-testnet=0 -regtest=0`), IPv4-only loopback RPC binding, and inbound-disabled peer settings (`-listen=0`, `-upnp=0`). Outbound networking is intentionally enabled for continuation checks (`-discover=1`, `-dnsseed=1`, `-dns=1`) and the script may connect to normal mainnet peers.
